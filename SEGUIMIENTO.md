...
- **Fase Jev (TypeSafe)**: J1 implementado el 2026-09-19 (`backend/jev.py` + endpoints `/api/jev*` + pagina UI "Jev AI" + hooks no bloqueantes en persist de scans/pipelines). **J0 validado el 2026-09-19** con pipeline 6 real: 22/25 aciertos, coste $0.0004/scan, umbral de confianza recomendado 0.5. Deshabilitado por defecto; key por UI o env `TYPESAFE_API_KEY`. Pendiente J2 (UI riesgo compuesto). Backup pre-cambio: tag `pre-jev-20260919`.
...
- **Pendiente (J2)**: badge "AI" + orden por riesgo compuesto en UI con umbral de confianza 0.5; J3 = export Splunk/PDF con veredicto. Deploy Railway: solo con env `TYPESAFE_API_KEY` si se decide habilitar, bajo responsabilidad individual.
