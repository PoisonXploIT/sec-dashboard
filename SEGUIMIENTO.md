...
- Siguiente del backlog OSINT: **`exploitdb_search`** (`wayback_urls` COMPLETO `a7762e3`).
...
*Última actualización: 2026-08-27, **cola de micro-pasos CERRADA**: rate limiting (`6df6c70`), paginación server-side (`e528101`), F1-FAVICON (`912676b`, cierra Fase 1), export CSV (`993ab45`) y **LOGGING ESTRUCTURADO** (`6e8e8c1`: `backend/applog.py`, RotatingFileHandler data/logs 5MBx3 + stdout, namespace sec_dashboard, LOG_LEVEL/LOG_DIR env, lifecycle scan/pipeline + M1 traceback vía _log.exception); **WAYBACK_URLS COMPLETO** (`a7762e3`): tool OSINT archive.org CDX en tools/web.py (matchType=domain + collapse urlkey, limit clamp 1000, paths agregadas, best-effort), adapter INFO, 9 tests sin red — siguiente del backlog OSINT: exploitdb_search; suite 221 tests en verde; ruff limpio.
