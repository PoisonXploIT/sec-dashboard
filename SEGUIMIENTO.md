...
- **Fase Jev (TypeSafe)**: J1 implementado el 2026-09-19 (`backend/jev.py` + endpoints `/api/jev*` + pagina UI "Jev AI" + hooks no bloqueantes en persist de scans/pipelines). **J0 validado el 2026-09-19** con pipeline 6 real: 22/25 aciertos, coste $0.0004/scan, umbral de confianza recomendado 0.5. Deshabilitado por defecto; key por UI o env `TYPESAFE_API_KEY`. **J2 COMPLETADO** (tarjeta "AI Verdicts" + orden por riesgo compuesto, E2E con API real: scan 47, $0.0002). **J3 COMPLETADO (2026-09-20)**: veredictos AI en exports (JSON/Splunk, CSV, PDF ejecutivo) + comparativa historica con prev/cur por finding; sin `result.jev.status=ok` los exports son byte-identicos a pre-J3. E2E con API real: scan 51/52 + pipelines 9/10, $0.0003 (7.686 tok entrada). Backup pre-cambio: tag `pre-jev-20260919`.
...
- **42 tools operativas** en Python puro (sin binarios externos), en `backend/tools/`:
...
| 5 | `publicwww_search` | `osint.py` | publicwww.com | No | 2 h | **COMPLETO** (`15e5b25`): hosts/tech/urls expuestos por dominio, adapter MEDIUM/INFO cap 10. Al verificar el servicio esta en mantenimiento (reintentar smoke). Ver footer. |

Alternativas del mismo nivel si alguna de las anteriores no encaja:
- `searchcode_search` (searchcode.com): **DEAD** — sitio pivoteado a MCP server de "code intelligence for LLMs"; la API anonima antigua (`/api/codes`) esta muerta (404, verificado vivo 2026-08-27); descartado. Ver seccion 6.
- `grepapp_search` (grep.app): **COMPLETO** — publico sin key (UA tipo navegador por el WAF de Vercel); ver seccion 6.
- `vulners_search` (vulners.com): **COMPLETO** — `VULNERS_API_KEY` por env; alternativa/complemento a ExploitDB; ver seccion 6.
- `otx_lookup` (otx.alienvault.com): **DEAD** — API v1 muerta (verificado vivo 2026-08-27: 404 en todos los endpoints); descartado. Ver seccion 6.
- `greynoise_lookup` (greynoise.io): **COMPLETO** (`55a6bec`) — publico sin key, `GREYNOISE_API_KEY` opcional; ver seccion 6.
- `urlscan_lookup` (urlscan.io): **COMPLETO** (`9a34ab9`) — ver entrada de la seccion 6.
- `securitytrails_enum` (securitytrails.com): DNS historico y subdominios.
- `hunter_email_finder` (hunter.io): **COMPLETO** (`55a6bec`) — `HUNTER_API_KEY` por env; ver seccion 6.
...
- Cola principal OSINT CERRADA (`publicwww_search` COMPLETO `15e5b25`; `dnsdumpster_enum` COMPLETO `fe2ca0e`; `shodan_lookup` COMPLETO `15c101c`; `exploitdb_search` COMPLETO `2a9dc52`; `wayback_urls` COMPLETO `a7762e3`).
...
### 2026-08-27 — Export CSV completo (cola de micro-pasos, paso 4)
- Commit `993ab45`. Dos endpoints nuevos: `GET /api/scans/{id}/export/csv` y `GET /api/pipelines/{id}/export/csv`, mismos patrones que los exports JSON/PDF existentes (fetch por id, 404 si no existe, target join, `Content-Disposition` attachment).
- Formato (csv stdlib, sin dependencias nuevas): **una fila por finding** con la metadata del run repetida en cada fila (cada fila autocontenida para ingesta SIEM). Columnas scan: `scan_id, tool, status, target_name, target_host, started_at, finished_at, elapsed_seconds, score` + `finding_id, severity, category, title, description, evidence, cve, confidence, remediation`. Pipeline: idem pero con `mode`, `total_tools` y columna `tool` por finding (identidad per-tool de la lista agregada). Encabezado: **UTF-8 con BOM + CRLF** (Excel abre limpio; csv round-trip para el resto). Run **sin findings = una fila resumen** con campos de finding vacios (el evento sigue visible para SIEM); findings NULL/corrupto legacy tolerados a [] (mismo patron que compare/paginacion).
- Frontend: `downloadCSV` (mismo mecanismo de ancla que JSON/PDF) + boton **CSV** en los 5 sitios de exports (tabla pipelines History, card detalle pipeline, tabla scans, tab Pipelines, card detalle scan) + filas en la API Reference de la guia in-app.
- Tests: 10 nuevos en `tests/test_report_csv.py` (sin red): BOM+CRLF+header, una fila por finding con metadata repetida, quoting round-trip de comas/newlines en evidence, run sin findings = fila resumen, NULL/corrupto tolerado, columnas pipeline (mode/total_tools/tool por finding), endpoints contra DB temporal (monkeypatch `models.DB_PATH`) con 404. Suite **204** en verde (194 + 10), ruff limpio, compileall OK, sintaxis JS validada con node.
- Siguiente micro-paso: **logging estructurado** (niveles + archivo rotativo en vez de print(); diagnostic real en produccion).

### 2026-08-27 — Logging estructurado completo (cola de micro-pasos, paso 5; CIERRE DE LA COLA)
- Commit `6e8e8c1`. `backend/applog.py` NUEVO: namespace de loggers `sec_dashboard.*` con dos handlers — `RotatingFileHandler` en `data/logs/sec-dashboard.log` (5 MB x 3, utf-8; la ruta ya estaba ignorada por `*.log` en .gitignore) + `StreamHandler` a stdout (Railway captura). Formato `timestamp | LEVEL | logger | message` (greppable). Config: env `LOG_LEVEL` (DEBUG/INFO/WARNING/ERROR, default INFO, invalido -> INFO) y `LOG_DIR` (default `data/logs`). `setup_logging()` reconfigura limpiando handlers previos (cualquier numero de llamadas = exactamente 1 file + 1 stream). Ruido de librerias (aiohttp/uvicorn) queda fuera del namespace.
- Sustituciones: prints `[startup]` de main.py -> `_log.info/warning` (migraciones, orphan cleanup, linea nueva `startup complete: N tools, remote_mode=`); prints `[splunk]` -> `_log.warning`; **M1 reforzado**: `traceback.print_exc()` de `scanner.run_tool` -> `_log.exception` (el traceback sigue siendo solo server-side).
- Nuevo diagnostico real en produccion: lifecycle completo — `scan started/finished/cancelled/failed` y `pipeline started/finished/cancelled/failed` con scan_id/pipeline_id, tool/mode, target, status, elapsed, score, findings/tools. La rama `except Exception` de pipeline (antes silenciosa) ahora hace `_log.exception`.
- CLI (`backend/cli.py`) intacto a proposito: su stdout/stderr es salida de programa, no logging; sin DB/web tampoco tiene motivo de archivo.
- Tests: 8 nuevos en `tests/test_applog.py` (sin red): handlers exactos + bounds de rotacion (5MB/3), idempotencia sin duplicados (una sola escritura por mensaje), formato con metadata en linea, LOG_LEVEL=DEBUG captura / default INFO descarta debug, nivel invalido -> INFO, resolve_log_dir env/arg/default, reconfiguracion cambia de directorio. Suite **212** en verde (204 + 8), ruff limpio, compileall OK.
- Smoke in-proceso: `asyncio.run(main.startup())` local — lineas a stdout Y archivo; `data/logs/sec-dashboard.log` creado con las migraciones + `startup complete: 41 tools registered`.
- **Cola de micro-pasos del usuario CERRADA** (rate limiting, paginacion, F1-FAVICON, export CSV, logging). Siguiente por la cola fijada: **backlog OSINT/threat intel — `wayback_urls` primero** (reglas y tabla en la seccion correspondiente de este archivo).

### 2026-08-27 — exploitdb_search completa (backlog OSINT, paso 2)
- Commit `2a9dc52`: tool `exploitdb_search` en `backend/tools/vuln.py` + registro TOOLS/SPECIAL_TOOLS/HANDLERS + adapter findings + 11 tests nuevos (`tests/test_tools_exploitdb.py`, sin red: stub de `_exploitdb_query`). Suite **232** en verde (221 + 11), ruff limpio, compileall OK.
- Fuente real (verificada contra el sitio): exploit-db.com NO expone API JSON pública sin key — los resultados del buscador salen de un endpoint **server-side de DataTables** (`GET /search` con `draw/start/length/columns[i][data|name|searchable]/search[...]/order[...]` + header `X-Requested-With: XMLHttpRequest`). El set de columns es EXACTO (8 columnas fijas): uno menos → `{"message": "Server Error"}`. Helper `_exploitdb_query(term, cve, limit)` monkeypatcheable, patrón `_cdx_query`: None = fallo → error dict best-effort.
- Routing: input = raw query (SPECIAL_TOOLS, igual que `cve_search`) — si el input entero es un CVE id (`CVE-\d{4}-\d+`, case-insensitive) va por `cve=` (mayúsculas), si no por `q=` (búsqueda por título). max_results clamp 1..200 (default 50), timeout 30 s, https.
- Salida: exploits con id/título/type/platform/author/date_published/verified/url + `cves` extraídos de la lista `code` (solo `code_type == cve` → `CVE-{code}`); orden por fecha desc (ISO = orden de string). Títulos y authors **unescape HTML** (`&#039;`, `&amp;`) — bug real cazado en smoke.
- Adapter: cada exploit = **HIGH** (el tool no confirma que el producto esté desplegado — la confianza lo matiza: verified 0.95 / unverified 0.75), cap 10, campo `cve` = primero de la lista, evidence con id/url/type/platform/date/verified, remediation de parche.
- Smoke in-proceso (red real): `run_tool('exploitdb_search', 'CVE-2014-0160')` → 5 entradas Heartbleed (papers + remote), verified correctas, cves extraídos, 5 findings HIGH, score 32.
- Observaciones registradas: (1) el endpoint XHR fue intermitente durante la inspección (algunos `q=`/`text=` con HTTP 000 y `cve=` siempre OK — parece rate-limit/WAF por ráfaga); best-effort lo cubre, si se degrada en prod se verá como error dict. (2) NO entra en pipelines (mismo criterio que wayback_urls/favicon): standalone vía UI/CLI (`--tool exploitdb_search --target "CVE-..."` o producto); meterlo en Full Depth = una línea en config.py si el usuario lo pide.
- Siguiente del backlog OSINT: **`shodan_lookup`** (REQUIERE API key — env/UI, 2-3 h).

### 2026-08-27 — shodan_lookup: decisión de diseño registrada (extensión, no tool nueva)
- No se tocó código. Se resolvió la duda de diseño del siguiente ítem del backlog OSINT.
- Hechos verificados en el repo: `shodan_lookup` YA existe en `backend/tools/osint.py` (free tier `internetdb.shodan.io/{ip}` sin key + `/shodan/host/{ip}` con env `SHODAN_API_KEY`), registrado en `TOOLS`/categoría OSINT de config.py, **sin adapter** en findings.py y fuera de todos los pipelines. El ítem del PDF es un superconjunto (dsearch query API: puertos/banners/CVEs/tags), no una tool nueva.
- Decisión: **EXTENDER el tool existente; NO renombrear**. Razones: (1) renombrar rompe datos persistidos (las filas de `scans`/`pipelines` guardan el nombre de la tool), referencias UI/CLI y tests, sin beneficio funcional; (2) misma fuente y mismo input (IP/dominio): dsearch es solo la vía API más rica cuando hay key; (3) la degradación graceful existente (sin key → internetdb) se conserva intacta.
- Diseño aprobado para la próxima sesión: ID `shodan_lookup` y registro intactos. Sin key → internetdb tal cual. Con `SHODAN_API_KEY` → `/shodan/dsearch?query=<ip>&fields=port,product,version,banner,vulns,tags,cpe,os,hostnames` (cap 100 resultados), fallback a `/shodan/host/{ip}` si dsearch devuelve vacío o falla. Helper `_shodan_query` monkeypatcheable (patrón `_cdx_query`/`_exploitdb_query`). Adapter `@register("shodan_lookup")` en findings.py desde el día uno: vulns/CVEs en banners = HIGH unverified (confidence ~0.75), puertos sensibles abiertos MEDIUM, tags/os INFO, cap 10. Tests sin red con stub de `_shodan_query`.
- Siguiente del backlog OSINT: **implementar la extensión de `shodan_lookup`** según este diseño.

### 2026-08-27 — shodan_lookup: extensión completa (dsearch con key)
- Commit `15c101c`: implementación exacta del diseño registrado, sin desviaciones. `backend/tools/osint.py`: helpers `_shodan_query`/`_shodan_host_query` monkeypatcheables (patrón `_cdx_query`, None = fallo → best-effort); con `SHODAN_API_KEY` → `/shodan/dsearch?query=<ip>&fields=port,product,version,banner,vulns,tags,cpe,os,hostnames` (cap 100) → filas normalizadas (banner truncado a 500, vulns solo strings, filas malformadas saltadas); dsearch vacío/fallo → fallback `/shodan/host/{ip}` (forma `services` intacta); ambos fallan → error dict. Sin key → rama internetdb **byte-identical** (cero riesgo). `import os` hoisted a módulo.
- `backend/findings.py`: adapter `@register("shodan_lookup")` — vulns de banner = HIGH unverified confidence 0.75 (dedup por cve, contexto port/producto/banner en evidence), puertos sensibles MEDIUM (mapa `_SENSITIVE_PORTS` extraído a nivel de módulo y compartido con el adapter `port_scanner` refactorizado), perfil os/tags INFO; orden vulns→puertos→perfil, cap 10. Las tres formas de resultado cubiertas: dsearch (`results`), /host (`services` + top-level) e internetdb (top-level `ports`/`vulns`/`os`/`tags`).
- `config.py`: descripción actualizada (InternetDB free tier; dsearch con key). ID/icono/timeout intactos.
...
- Cola principal OSINT CERRADA (`publicwww_search` COMPLETO `15e5b25`; `dnsdumpster_enum` COMPLETO `fe2ca0e`: la API nueva de dnsdumpster.com SI requiere key free, el endpoint anonimo antiguo estaba muerto; ver footer).

### 2026-08-27 — Fix dispatch: publicwww_search y dnsdumpster_enum sin handler (bug real cazado en el pase de evaluación)
- Commit `66f27f4`. Bug: `fe2ca0e` (dnsdumpster_enum) y `15e5b25` (publicwww_search) registraron la tool en `TOOLS`/config + adapter, pero NO tocaron `backend/scanner.py` — sin import ni entrada en `HANDLERS`. Consecuencia: ambas aparecian en `/api/tools`, UI y CLI (`--list-tools`) pero al ejecutar devolvian "unknown tool". **Correccion del registro**: el smoke publicwww del `15e5b25` ("error dict limpio, 0 findings") era ESE error de dispatch, no (solo) el mantenimiento del servicio — la llamada nunca llego a red. El hecho de que `api.publicwww.com` no resuelva DNS al verificar sigue siendo cierto y el best-effort sigue cubriendolo; reintentar smoke cuando vuelva.
- Fix: import + entrada en `HANDLERS` para ambas. Test de coherencia **inversa** en `tests/test_config.py` (`test_every_registered_tool_has_handler`: TOOLS -> HANDLERS) — la guardiana que lo cubre (la existente era solo HANDLERS -> TOOLS). Suite **267** en verde (266 + 1), ruff limpio, compileall OK.
- Smoke post-fix (red real): `run_tool('publicwww_search','sammideblas.com')` → ahora SI llega al handler: error dict best-effort por API caida (correcto mientras dure el mantenimiento); `run_tool('dnsdumpster_enum','sammideblas.com')` sin key → error dict "requires DNSDUMPSTER_API_KEY" limpio.
- Siguiente: alternativas del backlog OSINT a eleccion del usuario (mismas reglas) o cierre.

### 2026-08-27 — urlscan_lookup completa (alternativa OSINT, elegida por el usuario)
- Commit `9a34ab9`. Tool `urlscan_lookup` en `backend/tools/osint.py` + TOOLS/config (OSINT, icon UL, timeout 60) + HANDLERS scanner + adapter findings + 12 tests sin red (`tests/test_tools_urlscan.py`). Suite **279** en verde (267 + 12), ruff limpio, compileall OK.
- Fuente verificada EN VIVO antes de escribir el parser: `GET https://urlscan.io/api/v1/search/?q=domain:{domain}` sin key. Dos sorpresas del contrato real: (1) el param `limit` esta PROHIBIDO en peticiones anonimas (400 "limit is not allowed") — nunca se envia; (2) la respuesta anonima NO trae tech/status/listeners como documentacion vieja — solo `results[]` con `task` (domain/time/uuid), `canonical` (task.url = host escaneado; page.url = PATH post-redirect tipo "github.com/login" y por eso NO entra en hosts) y `page` (ip/server/country/tls...). Hosts salen de `task.domain` + `canonical.task.url`, dedup+cap 1000, filas malformadas saltadas, best-effort error dict. Sin deps nuevas.
- Adapter `@register("urlscan_lookup")`: MEDIUM nombre sensible (reutiliza `_sensitive_sub`, conf 0.7) + INFO perfil cap 10 — mismo criterio que dnsdumpster/publicwww.
- Smoke in-proceso (red real): `run_tool('urlscan_lookup','sammideblas.com')` → 3 scans, hosts sammideblas.com + sec.sammideblas.com, 1 finding INFO, score 0. Tambien valida el dispatch tras el fix `66f27f4`.
- Siguiente: resto de alternativas a eleccion (otx con key free, greynoise, hunter) o cierre; hardware sigue requiriendo capturas reales.

### 2026-08-27 — greynoise_lookup + hunter_email_finder completos (alternativas OSINT) + OTX descartado
- Commit `55a6bec`. Dos tools nuevas en `backend/tools/osint.py` + TOOLS/config (OSINT, iconos GN/HE, timeouts 15/30) + HANDLERS scanner + adapters findings + 23 tests sin red (`tests/test_tools_greynoise.py`, `tests/test_tools_hunter.py`). Suite **302** en verde (279 + 23), ruff limpio, compileall OK.
- **OTX descartado (API muerta)**: verificado en vivo — todos los endpoints de `otx.alienvault.com/api/v1/*` devuelven `404 {"detail": "endpoint not found"}` (los paths v2+ caen al catch-all del SPA Angular). Mismo destino que el endpoint anonimo antiguo de dnsdumpster. Fuera de alcance; si OTX reviviera la API, reabrir.
- **greynoise_lookup**: `GET https://api.greynoise.io/v3/community/{ip}`. Contrato verificado EN VIVO: (1) funciona SIN key (endpoint publico, ~25 req/min — decision registrada: el tool NUNCA exige key; `GREYNOISE_API_KEY` opcional via header `key` solo sube el rate limit); (2) 404 = cuerpo JSON valido de "no data" (`noise:false, riot:false, message`) y NO es error; (3) 401/400/429 = cuerpos `{"message": ...}` sin keys noise/riot → error dict. Target host/IP (resuelve como shodan). Adapter: noise+malicious = HIGH conf 0.8, noise benign = LOW conf 0.7, RIOT solo = INFO perfil; found:false/error = 0 findings; cap 1.
- **hunter_email_finder**: `GET https://api.hunter.io/v2/domain-search?domain=<d>` con `Authorization: Bearer <key>`. **El endpoint antiguo `/v2/domain-account` esta muerto (404 HTML branded)** — verificado en vivo y contrato corregido contra la doc oficial (`hunter.io/api-documentation/v2`, descargado y parseado). Respuesta documentada: `data.emails[]` con `value/type/confidence/first_name/last_name/position/seniority/department/decision_maker` + `meta.results`; free plan cap 10 por llamada. Sin key → error dict limpio SIN red (mismo criterio que dnsdumpster). Parser: dedup por valor normalizado, campos mapeados, cap 100. Adapter: local part sensible (`admin/root/ciso/ceo/director/it/security/backup/git`) o `decision_maker` = MEDIUM conf 0.7 + INFO perfil, cap 10.
- Smoke final contra sammideblas.com (todo el backlog OSINT via run_tool): greynoise live sin key → `found:false` limpio en 0.9s (IP de Cloudflare, no observada como scanner); hunter sin key → error dict limpio; urlscan count=3 (hosts apex + sec.); shodan internetdb real data (score 4); degradaciones externas limpias: crt.sh timeout (ct_logs), archive.org CDX y api.publicwww.com caidos del momento (error dicts best-effort, no bugs).
- Siguiente: hardware con capturas reales del usuario (B WiFi Marauder v6/CYD reutiliza `wifi.py`; C Bruce y D HackRF/BusPirate requieren capturas) o alternativas restantes del backlog (`searchcode`, `grep.app`, `vulners`; `securitytrails` = pagada, fuera).

### 2026-08-27 — Alternativas restantes agotadas: grepapp_search + vulners_search completos, searchcode descartado
- Commits `3e8f457` (feat) + docs. Dos tools nuevas (`backend/tools/osint.py`, `backend/tools/vuln.py`) + TOOLS/SPECIAL_TOOLS/HANDLERS + adapters findings + 22 tests sin red.
- **searchcode_search descartado**: verificado en vivo — searchcode.com ha pivoteado a un MCP server de "code intelligence for LLMs" (analisis estructurado sobre repos git publicos, orientado a tokens); la API anonima antigua (`/api/codes?language=&q=`) esta muerta (404 plano "404 page not found", estilo Go; `/api`, `/mcp` y hasta `/robots.txt` devuelven 404). Mismo destino que OTX y el endpoint anonimo antiguo de dnsdumpster. Fuera de alcance; si reviviera una API publica de busqueda, reabrir.
- **grepapp_search**: `GET https://grep.app/api/search?q=<term>` SIN key. Contrato verificado EN VIVO: top `{time, facets:{lang:{buckets:[{val,count}]}}, hits:{total, hits[]}}`; hit = `{repo ("owner/repo"), path (archivo en el repo), branch, total_matches (int o "100+"), content:{snippet HTML}}` — sin matched_lines ni language por hit (los idiomas salen de facets). **El Vercel Security Checkpoint responde 429 a User-Agents no-navegador** → la peticion usa UA tipo navegador (decision registrada; si el WAF se endurece, best-effort degrada a error dict).
- Routing grepapp: query crudo (SPECIAL_TOOLS) — dominio, email o prefijo de token. max_results clamp 1..100 (default 50). Parser: dedup por (repo, path) normalizado, filas malformadas saltadas, languages desde facets cap 20.
- Adapter grepapp: nombres de archivo sensibles (`.env`/`.env.*`, `.pem/.key/.p12/.pfx/.jks/.keystore/.netrc`, `id_rsa/id_dsa/id_ecdsa/id_ed25519`, `credentials`/`secrets`) = MEDIUM conf 0.7 + INFO perfil (count/total/top/languages), cap 10.
- **vulners_search**: `GET https://api.vulners.com/v3/vuln` con header `X-Vuls-API-KEY`. Contrato por doc oficial (best-effort: sin key no hay test en vivo — el acceso anonimo cae en un challenge Cloudflare "Just a moment" 403): `{"meta", "results":[{id,title,type,severity,description,published,modified,references,...}]}`. Routing: query crudo (SPECIAL_TOOLS) — CVE id suelta → `advisory=`, resto → `search=`; max_results clamp 1..100 (default 50). Sin key → error dict limpio SIN red (criterio dnsdumpster). Parser: dedup por id, unescape HTML en title, description cap 500, url `vulners.com/{type}/{id}`, orden published desc.
- Adapter vulners: critical/high = HIGH conf 0.75, medium = MEDIUM 0.7, low/unknown = LOW (0.6/0.5); dedup por id, cap 10.
- Tests: 22 nuevos sin red (11 por tool en `tests/test_tools_grepapp.py` y `tests/test_tools_vulners.py`). Suite **324** en verde (302 + 22), ruff limpio, compileall OK.
- Smoke via run_tool: grepapp `sammideblas.com` → 0 hits limpio en 0.6s; grepapp `@example.com` → datos reales (total ~379k, clamp 10 repos, perfil INFO 1 finding score 0); vulners sin key → error dict en 0.0s (cero red).
- **ALTERNATIVAS AGOTADAS**: searchcode muerto, otx muerto, securitytrails pagada (fuera). El backlog OSINT/threat intel queda CERRADO de verdad. Siguiente: hardware con capturas reales del usuario (B WiFi Marauder v6/CYD reutiliza `wifi.py`; C Bruce y D HackRF/BusPirate requieren capturas) o backlog pendiente (`--save` CLI, TUI, tests de integracion end-to-end, config por env).

### 2026-08-27 — Plan de seguridad (bunker): fase commiteada 2.1/2.3/2.4/2.5 + bug F821 cazado por ruff
- Commit `0ae4c44` (feat). Trabajo del working tree de la sesión interrumpida (contexto renovado a medias); todo pertenece al **plan de seguridad para el dashboard expuesto a internet** (orden acordado: export CSV → logging estructurado → test masivo contra sammideblas.com → plan de seguridad).
- **2.1 Autenticación**: `backend/authguard.py` NUEVO — `truncate_key()` (8 primeros chars + "..."; keys cortas 2 chars; NUNCA el secreto completo) y `FailedAuthTracker` (lockout tras N fallos en ventana: env `SEC_AUTH_MAX_FAILURES`=5 / `SEC_AUTH_FAILURE_WINDOW`=300s / `SEC_AUTH_LOCKOUT`=900s; un success limpia contador Y lockout activo — coste documentado: un origin compartido de Cloudflare no queda rehén de un typo). Cableado en `require_api_key`: bloqueado → **403 + Retry-After ANTES del trabajo de auth**; fallo → WARNING con key truncada; ok → DEBUG. **Rate limit por API key**: buckets `_key_mutation_limiter`/`_key_read_limiter` (mismos límites 30/600) keyed por `k:sha256(presented)[:32]` — el secreto crudo no entra a memoria más allá del hash; sin key → solo cubre el bucket IP. **WebSocket verificado** (ítem "Revisar" del plan): `/ws` ya exige `?key=` o Cloudflare Access si `API_KEY` set (preexistente en HEAD, no se tocó).
- **2.3 Datos**: `backend/maintenance.py` NUEVO — `purge_old_runs(db, retention_days)` (env `SEC_DASHBOARD_RETENTION_DAYS`=30, 0 desactiva; devuelve conteos) + `backup_db(src, dir, keep)` (env `SEC_DASHBOARD_BACKUP_KEEP`=7; copia a `data/backups` con timestamp UTC y poda las más viejas). `_maintenance_loop` en main.py: primer pase en startup y luego cada 6h.
- **2.4 Auditoría**: logs de auditoría — `audit scan created` / `audit pipeline created` (timestamp, key truncada, target, tool/mode, IP); alerta fallo encadenado: env `SEC_ALERT_CONSECUTIVE_FAILURES`=5 scans fallidos seguidos → ERROR "ALERT ..." (la línea de log = hookpoint; sin canal externo, los webhooks del usuario son para eventos de scan).
- **2.5 Despliegue**: Dockerfile non-root (`appuser`, owns `/app/data`); job CI `dependency-audit` (`pip-audit -r requirements.txt`); `.github/dependabot.yml` (pip + github-actions, PRs semanales).
- **Bug cazado por ruff ANTES del commit**: el audit log de scan usaba `effective_target` — nombre indefinido en el scope de `create_scan` (F821) que habría crasheado cada creación de scan. Fix: `target_host or "(none)"`. Lección registrada: el trabajo bunker llegó al working tree sin ruff corriendo; la suite verde no cubre ese path.
- **Fix tests/test_maintenance.py**: `_seed` insertaba scans/pipelines con `target_id=1` sin fila en `targets` → IntegrityError (`get_db()` activa `PRAGMA foreign_keys=ON`); y al lanzar, la conexión aiosqlite quedaba sin cerrar → worker thread non-daemon vivo → **pytest colgado al salir tras imprimir el summary** (causa del run de 1073s). Fix: crear un target real primero + close en try/finally (endurecidos también los coroutines internos).
- Suite **339** en verde ~1.3s, ruff limpio, compileall OK.
- Pendiente / siguiente: network policies (limitar egress — lado deploy, Railway), encriptación DB en reposo (opcional, fuera de MVP), y el ítem 3 del orden acordado: **test masivo contra sammideblas.com** con la plantilla; luego reparar tools bloqueadas/timeouts según resultados.

### 2026-08-28 — Test masivo contra sammideblas.com + push del plan de seguridad
- Commits `b7665a3` (docs) + docs de cierre. Push a origin/master hecho tras test masivo; CI verde, Railway desplegado.
- **Test masivo** (ítem 3 del orden acordado):
  - Pipeline full_depth: ~15s, 5 tools operativas, score 0 (solo findings INFO).
  - Pipeline nuclear: ~20s, 25 tools operativas, score 18/100. Findings reales: port 8080 MEDIUM (port_scanner + shodan_lookup concordantes), headers faltantes LOW (header_analyzer). Degradación graceful en asn_lookup (bgpview.io unreachable desde este entorno, fallback ip-api OK).
  - SPECIAL_TOOLS operativas: hash_checker (OK con formato HIBP válido), password_audit (OK contra HIBP, Weak/breached para "password123"), cve_search (CVE-2014-0160 → HIGH CVSS 7.5), exploitdb_search (CVE-2014-0160 → 5 exploits Heartbleed), grepapp_search (@example.com → ~380k hits), vulners_search (error dict limpio "VULNERS_API_KEY not set" sin key configurada).
- **Ítem 4 según resultados**: ninguna herramienta bloqueada ni con timeouts anormales; no requiere reparación ni ajuste.
...
- **J0 COMPLETADO (2026-09-19)** con pipeline 6 real (25 findings): 22/25 coinciden con juicio humano; los 3 discordantes llevan confianza 0.36-0.42 (umbral 0.5 los enviaria a revision, no a accion). Placeholders "no dedicated adapter": 12/15 detectados como noise correctamente. Coste real $0.0004 (9.377 tok entrada). Veredicto: **PASA**. Tabla de validacion rellenada en la nota de brainstorming.
- **J2 COMPLETADO (2026-09-19/20)**: tarjeta "AI Verdicts (Jev)" en vista de scan y pipeline (`jevVerdictsHtml`), ordenada por riesgo compuesto = `SEVERITY_WEIGHT[severity] x severity_score`; badge segun umbral validado: c>=0.5 -> verdict (tag-red/green/plain), 0.3-0.5 -> "AI: revisar" (tag-yellow), <0.3 -> oculto. Constante `JEV_CONFIDENCE_THRESHOLD = 0.5` en el JS. **E2E con API real**: server local :8799 + key por env, `POST /api/scans` header_analyzer contra example.com -> scan 47 persistido con `result.jev.status=ok`, 11 verdicts por finding_id (HSTS/CSP true_positive c=0.97/0.98), coste $0.0002 (4.288 tok). JS inline validado con `node --check`; 383 tests.
- **Pendiente (J3)**: export Splunk/PDF con veredicto. Deploy Railway: solo con env `TYPESAFE_API_KEY` si se decide habilitar, bajo responsabilidad individual.

### 2026-09-20 — Fase J3: veredictos AI en exports y comparativa historica (J3 COMPLETADO)
- **Alcance**: (1) campo nuevo con el veredicto de Jev en export JSON/Splunk, CSV y PDF ejecutivo; (2) comparativa historica (`/api/pipelines/compare` + evolucion UI) con "verdict changed" por finding.
- **Regla de diseno**: sin `result.jev.status == "ok"`, todos los exports son byte-identicos al output pre-J3 (misma regla que J2 en la UI).
- **backend/report.py**: `AI_CSV_FIELDS` (`ai_verdict`, `ai_confidence`, `ai_severity_score`, `ai_immediate_action`) al final del header CSV; exports JSON (scan/pipeline/all) ganan bloque `"ai"` (`model` + `verdicts[]` unido por finding_id, fallback a indice para ids legacy); PDF ejecutivo con seccion "AI Verdicts (Jev)" entre Top Findings y Heatmap: model, findings evaluados, distribucion de veredictos, input tokens, tabla top 15 ordenada por riesgo compuesto (`SEVERITY_WEIGHT x severity_score`).
- **backend/main.py**: `/api/pipelines/compare` selecciona ahora `result` y parsea jev por run: items new/fixed llevan `ai_verdict`/`ai_confidence`, persistent lleva `ai_verdict_prev`/`ai_verdict_cur` (la UI marca CAMBIO). Claves solo si hay datos. Fix bonus: `POST /api/scans` con `wait=True` devuelve ahora el dict persistido exacto (`stored`) para que la respuesta sincrona incluya `result.jev`.
- **frontend/index.html**: tag AI por item en el delta de evolucion (new/fixed: veredicto + confianza; persistent: prev -> cur con "(CAMBIO)" o "(igual)"). Help text actualizado.
