...
*Última actualización: 2026-08-27, **cola de micro-pasos CERRADA**: rate limiting (`6df6c70`), paginación server-side (`e528101`), F1-FAVICON (`912676b`, cierra Fase 1), export CSV (`993ab45`) y **LOGGING ESTRUCTURADO** (`6e8e8c1`); **WAYBACK_URLS COMPLETO** (`a7762e3`): tool OSINT archive.org CDX en tools/web.py, adapter INFO, 9 tests sin red; **EXPLOITDB_SEARCH COMPLETO** (`2a9dc52`): tool de exploits publicos por producto/CVE en tools/vuln.py (endpoint JSON server-side de DataTables de exploit-db.com — set exacto de 8 columns, routing `cve=`/`q=`, clamp 1..200, unescape HTML, best-effort), adapter HIGH cap 10 con confidence por verified y cve desde code list, 11 tests sin red; suite **232** tests en verde; ruff limpio. **SHODAN_LOOKUP EXTENDIDO** (`15c101c`): dsearch con `SHODAN_API_KEY` (cap 100, fields port/product/version/banner/vulns/tags/cpe/os/hostnames) + fallback `/host`; internetdb sin key intacto; adapter HIGH/MEDIUM/INFO cap 10 con `_SENSITIVE_PORTS` compartido; suite **242** en verde. **DNSDUMPSTER_ENUM COMPLETO**: tool `dnsdumpster_enum` (tools/network.py) contra la API nueva de dnsdumpster.com (`api.dnsdumpster.com/domain/{domain}`, header `X-API-Key`) — el endpoint anonimo antiguo (`api.hackertarget.com/dnsdumpster.php`) esta muerto (404); requiere `DNSDUMPSTER_API_KEY` por env (cuenta gratis, 50 records; decision del usuario: se deja para quien quiera poner key, mismo criterio que SHODAN_API_KEY). Parser `_dnsdumpster_hosts` (a/cname/mx/ns → hostnames), dedup+cap 1000, best-effort error dict. Adapter MEDIUM (subdominios con nombre sensible por label: staging/dev/test/backup/git...) + INFO perfil, cap 10. Smoke sammideblas.com sin key: error limpio, 0 findings. **PUBLICWWW_SEARCH COMPLETO** (`15e5b25`): tool `publicwww_search` (tools/osint.py) contra `api.publicwww.com/domains/{domain}` (sin key), helper `_publicwww_query` monkeypatcheable, agregacion hosts/technologies/urls (dedup, caps 1000/100/200, filas malformadas saltadas), best-effort error dict; adapter MEDIUM nombre sensible (reutiliza `_sensitive_sub` del adapter dnsdumpster) + INFO perfil cap 10; 11 tests sin red; suite **266** en verde. AVISO: al verificar el servicio esta EN MANTENIMIENTO (`api.publicwww.com` no resuelve DNS, el sitio muestra "under maintenance") → smoke sammideblas.com = error dict limpio, 0 findings (best-effort cubierto); reintentar smoke cuando vuelva. **COLA PRINCIPAL OSINT CERRADA** (wayback, exploitdb, shodan, dnsdumpster, publicwww); pendientes: alternativas (`searchcode`, `grep.app`, `vulners`, `otx`, `greynoise`, `urlscan`, `securitytrails`, `hunter.io`) a eleccion del usuario. **FIX DISPATCH** (`66f27f4`): publicwww_search y dnsdumpster_enum quedaban sin handler en scanner.HANDLERS (fe2ca0e/15e5b25 no tocaron scanner.py) — "unknown tool" al ejecutar; el smoke publicwww del 15e5b25 era ese error dict, no solo el mantenimiento; test de coherencia inversa TOOLS->HANDLERS; suite **267**. **URLSCAN_LOOKUP COMPLETO** (`9a34ab9`): scan pasivo (hosts) via API anonima de urlscan.io — `limit` prohibido en peticiones sin key (verificado vivo), hosts desde task.domain + canonical.task.url, adapter MEDIUM/INFO cap 10; suite **279**; smoke sammideblas.com OK (3 scans). **GREYNOISE_LOOKUP + HUNTER_EMAIL_FINDER COMPLETOS** (`55a6bec`): OTX descartado (API v1 muerta, verificado vivo); greynoise publico sin key (header `key` opcional solo sube rate limit), 404 = no-data JSON valido, adapter HIGH/LOW/INFO cap 1; hunter contra `/v2/domain-search` (el antiguo `domain-account` esta muerto) con Bearer, sin key error dict sin red, MEDIUM local-part-sensible/decision-maker + INFO perfil cap 10; suite **302** en verde; smoke sammideblas.com todo limpio. **GREPPAPP_SEARCH + VULNERS_SEARCH COMPLETOS / SEARCHCODE DESCARTADO** (ver entrada de la seccion 6): grepapp publico sin key con UA tipo navegador (WAF de Vercel), contrato verificado vivo, adapter MEDIUM archivo-sensible + INFO perfil cap 10; vulners con `VULNERS_API_KEY` por env (anonimo = challenge Cloudflare), routing advisory=/search=, adapter HIGH/MEDIUM/LOW cap 10; searchcode.com pivoteado a MCP server y su API anonima muerta (404 verificado vivo); suite **324** en verde; smoke sammideblas.com limpio. **ALTERNATIVAS AGOTADAS** (otx/searchcode muertos, securitytrails pagada): el backlog OSINT queda cerrado. Siguiente: hardware con capturas reales o backlog pendiente (`--save` CLI, TUI, tests end-to-end, config por env).
...
**F1-FAVICON HECHO** (`912676b`, cierra Fase 1): tool `favicon_fingerprint` (modulo propio `backend/tools/favicon.py`) — GET pasivo de 4 paths de icono, MD5/SHA256, lookup contra `data/favicon_hashes.json` (6 favicons oficiales REALES: WordPress, Drupal, Joomla, Shopify, Ghost, Wix); findings INFO; no entra en pipelines (extension registrada); renderer PDF por fallback JSON; base extensible editando el JSON.
**EXPORT CSV HECHO** (`993ab45`): ver entrada de la seccion 6 — dos endpoints csv stdlib con una fila por finding + metadata del run, BOM UTF-8+CRLF, fila resumen si sin findings. Suite 204.
**Export CSV HECHO** (`993ab45`): `GET /api/scans/{id}/export/csv` + `GET /api/pipelines/{id}/export/csv` — csv stdlib, una fila por finding con metadata del run autocontenida (SIEM), BOM UTF-8 + CRLF (Excel), run sin findings = fila resumen, NULL/corrupto tolerado; botones CSV en 5 sitios UI + filas guia. Suite 204.
**LOGGING ESTRUCTURADO HECHO** (`6e8e8c1`): `backend/applog.py` — namespace `sec_dashboard.*`, RotatingFileHandler `data/logs/sec-dashboard.log` (5MB x3) + stdout, env `LOG_LEVEL`/`LOG_DIR` (fallback INFO), `setup_logging()` idempotente en el import de main.py; prints de startup/splunk sustituidos, M1 traceback via `_log.exception` en scanner, lifecycle scan/pipeline started/finished/cancelled/failed con ids/tool/mode/target/status/elapsed/score. CLI intacto (stdout = salida de programa). Suite 212.
**`wayback_urls` COMPLETO** (`a7762e3`). **`exploitdb_search` COMPLETO** (`2a9dc52`, ver entrada de la seccion 6): endpoint DataTables server-side (8 columns exactas, `cve=`/`q=`, unescape HTML), adapter HIGH cap 10. **shodan_lookup EXTENDIDO** (`15c101c`, ver entrada de la seccion 6): dsearch con key + fallback /host, adapter desde el dia uno. **`dnsdumpster_enum` COMPLETO** (ver footer): API nueva con `DNSDUMPSTER_API_KEY` por env (endpoint anonimo antiguo muerto); adapter MEDIUM/INFO cap 10. **`publicwww_search` COMPLETO** (`15e5b25`, ver footer): hosts/tech/urls expuestos, adapter MEDIUM/INFO cap 10; servicio en mantenimiento al verificar (smoke = error dict limpio). **COLA PRINCIPAL OSINT CERRADA**; quedan las alternativas de la seccion 'Backlog de integraciones OSINT / threat intel' (`searchcode`, `grep.app`, `vulners`, `otx`, `greynoise`, `urlscan`, `securitytrails`, `hunter.io`) a eleccion del usuario, con las mismas reglas fijadas: keys por env/UI, tests sin red, adapter desde el dia uno, best-effort. Candidatos fuera del orden estricto: tests de integración end-to-end (httpx.AsyncClient(app=main.app) + DB temporal), config por variables de entorno (PORT, API_KEY, DB_PATH, REMOTE_MODE, SPLUNK_*). Tras la cola: hardware/TUI con capturas reales — B WiFi Marauder v6 / CYD (reutiliza `wifi.py`, 1-2 h); C Bruce RF/BLE (`bruce.py` nuevo, REQUIERE capturas reales del serial/formato); D HackRF / Bus Pirate (parsers `.c16` e I2C/SPI/UART, capturas obligatorias). Si se elige hardware C/D: pedir las capturas ANTES de escribir parsers.
