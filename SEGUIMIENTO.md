...
| 3 | `shodan_lookup` | `osint.py` | shodan.io | Si (opcional) | 2-3 h | **YA EXISTE como tool** (internetdb sin key + `/shodan/host` con key). El ítem del PDF es una **EXTENSIÓN**, no una tool nueva: ver decisión registrada en la entrada 2026-08-27 de la sección 6. |
...
- Siguiente del backlog OSINT: **extensión de `shodan_lookup`** (dsearch con key; decisión registrada en la entrada 2026-08-27 de la sección 6).
...
*Última actualización: 2026-08-27, **cola de micro-pasos CERRADA**: rate limiting (`6df6c70`), paginación server-side (`e528101`), F1-FAVICON (`912676b`, cierra Fase 1), export CSV (`993ab45`) y **LOGGING ESTRUCTURADO** (`6e8e8c1`); **WAYBACK_URLS COMPLETO** (`a7762e3`): tool OSINT archive.org CDX en tools/web.py, adapter INFO, 9 tests sin red; **EXPLOITDB_SEARCH COMPLETO** (`2a9dc52`): tool de exploits publicos por producto/CVE en tools/vuln.py (endpoint JSON server-side de DataTables de exploit-db.com — set exacto de 8 columns, routing `cve=`/`q=`, clamp 1..200, unescape HTML, best-effort), adapter HIGH cap 10 con confidence por verified y cve desde code list, 11 tests sin red; suite **232** tests en verde; ruff limpio. Siguiente del backlog OSINT: **extensión de `shodan_lookup`** (ver entrada 2026-08-27 de la sección 6).
