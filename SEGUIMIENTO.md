...
*Última actualización: 2026-08-27 — **PLAN DE SEGURIDAD (BUNKER) COMMITTEADO** (`0ae4c44`): 2.1 lockout auth + rate limit por API key (keyed sha256(presented)) + logs de autenticación con key truncada; 2.3 purge retention (`SEC_DASHBOARD_RETENTION_DAYS`=30) + backup DB keep 7 cada 6h (`data/backups`); 2.4 audit log scan/pipeline created + alerta fallo encadenado (`SEC_ALERT_CONSECUTIVE_FAILURES`=5, ERROR log = hookpoint); 2.5 Docker non-root appuser + CI pip-audit + dependabot (pip y github-actions weekly). Env nuevas: `SEC_AUTH_MAX_FAILURES` / `SEC_AUTH_FAILURE_WINDOW` / `SEC_AUTH_LOCKOUT`. Suite **339** en verde ~1.3s, ruff limpio. Pendiente: network policies (egress, lado deploy Railway) y encriptación DB en reposo (opcional, fuera de MVP). Siguiente por orden acordado del usuario: test masivo contra sammideblas.com.*

**PROMPT PARA LA PRÓXIMA SESIÓN** (arrancar con contexto nuevo):
Eres el agente de sec-dashboard. Repo `C:\Users\Sammi\sec-dashboard` (origin/master = github.com/PoisonXploIT/sec-dashboard, deploy Railway en sec.sammideblas.com). **Lee primero este archivo entero**: la sección 7 son reglas NO VIOLAR y las secciones 4-5 documentan las convenciones de la suite (tests sin red, stubs/monkeypatch, `asyncio.run()` dentro de tests síncronos, DB temporal con monkeypatch de `models.DB_PATH`).

**Estado**: todo el backlog OSINT está cerrado (ver sección 6 para commits). **Plan de seguridad (bunker) commiteado en `0ae4c44`**: 2.1 authguard.py (FailedAuthTracker lockout + truncate_key, cableado en require_api_key con 403+Retry-After y logs WARNING/DEBUG key truncada; rate limit por API key con buckets `_key_*` keyed por `k:sha256(presented)[:32]`), 2.3 maintenance.py (purge retention + backup DB, loop cada 6h desde startup), 2.4 audit logs scan/pipeline created + alerta N fallos consecutivos, 2.5 Docker non-root + CI pip-audit + dependabot. WS auth verificado (ya exigía `?key=` o Cloudflare Access). **AVISO: `0ae4c44` está en local y NO push-eado** — el push despliega a producción; confirmar con el usuario antes de push (regla 8).

**Siguiente micro-paso (orden acordado por el usuario)**: ítem 3 — **test masivo contra sammideblas.com** (pipeline `full_depth` o `nuclear`, p. ej. CLI `python -m backend.cli --target sammideblas.com --pipeline full_depth --json` o vía web). Ítem 4 según resultados: reparar tools bloqueadas o ajustar timeouts. Después, lo que queda del plan de seguridad: **network policies** (limitar egress — lado deploy/Railway, no código) y **encriptación DB en reposo** (opcional, fuera de MVP; SQLite no lo hace por defecto). Plan de respuesta es OPERATIVO (sin código): escaneo masivo no autorizado → rotar API key; Railway cae → health check + alerta; finding con secret real filtrado → revocar/rotar inmediatamente.

Reglas: MVP sin dependencias nuevas, tests sin red, commits `feat:`/`fix:` largos y descriptivos, push solo con suite verde y confirmado, sin emojis ni símbolos de color. Comandos: `.venv\Scripts\python.exe -m pytest -q` (~1.3s), `-m ruff check backend tests`, `-m compileall -q backend`. Al cerrar sesión: añadir entrada al FINAL de la sección 6 (append-only) y actualizar este footer con el siguiente micro-paso.
