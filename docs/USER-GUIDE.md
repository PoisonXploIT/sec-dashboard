...
- **PS Security Audit coverage:** the enterprise audit's threats (`AMENAZAS_DETECTADAS.json`) are mapped to standard findings, so Jev verdicts and local LLM explanations apply to it exactly like any other tool. A clean audit (no threats) produces a single INFO finding.
...
- **Per-run selection (J6):** when you launch an individual tool (target picker) or a pipeline, an an *AI (optional, per run)* block appears with one checkbox per configured service — *Jev AI verdicts* and/or *Local LLM explanations (reference only)* — checked by default. Uncheck to run that scan/pipeline without it; the option applies to that run only, never to the global config.
...
- **UI view:** the scan/pipeline result shows a *Local LLM Explanations (reference only)* card below the AI Verdicts table.
- **PDF:** a *Local LLM Explanations (reference only)* section inside the AI Verdicts page.
